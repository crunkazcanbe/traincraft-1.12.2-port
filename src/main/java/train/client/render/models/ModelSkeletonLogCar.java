	...
	public void render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5)
	{
		...
		if(freight.getAmmountOfCargo() > 0) {
			for (ItemStack item : freight.cargoItems) {
				...
				if (item!=null && redwoods.contains(item.getTranslationKey())) {
					...
					GL11.glPopMatrix();
					break;
				}else if(item!=null && (item.getTranslationKey().equals("tile.log.spruce"))){
					...
					GL11.glPopMatrix();
					break;
				}else if(item!=null && (item.getTranslationKey().equals("tile.log.birch"))){
