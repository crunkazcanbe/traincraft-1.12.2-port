...
//test15
	...
	public void onGameRender(RenderGameOverlayEvent.Text event){
		if (game != null && game.player != null && game.player.getRidingEntity() != null && game.player.getRidingEntity() instanceof Locomotive && Minecraft.isGuiEnabled() && game.currentScreen == null) {
	...
	public static void drawTexturedRect(ResourceLocation texture, double x, double y, int u, int v, int width, int height, int imageWidth, int imageHeight, double scale) {
		...
		bufferbuilder.pos(x + scale*(double)width, y + scale*(double)height, 0).tex(maxU, maxV).endVertex();
		bufferbuilder.pos(x + scale*(double)width, y, 0).tex(maxU, minV).endVertex();
		bufferbuilder.pos(x, y, 0).tex(minU, minV).endVertex();
		bufferbuilder.pos(x, y + scale*(double)height, 0).tex(minU, maxV).endVertex();
		tessellator.draw();
	}
}
