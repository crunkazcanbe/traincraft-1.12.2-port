...
public class GuiLantern extends GuiScreen {
	...
	public void initGui() {
		...
		this.colorTextField = new GuiTextField(0, this.fontRenderer, this.width / 2 - 150, 60, 300, 20);
		...
		if (par1GuiButton.enabled) {
			...
			if (par1GuiButton.id == 0) {
				...
				if(colorString.length()==6){
					...
					if(colorString.length()==6 && tryParse(colorString)!=null){
						int color = tryParse(colorString);
						net.minecraft.util.math.BlockPos pos = lanternBlock.getPos();
						Traincraft.modChannel.sendToServer(new PacketLantern(color, pos.getX(), pos.getY(), pos.getZ()));
