...
public class GuiDistil extends GuiContainer {
	...
	public void drawScreen(int t, int g, float par3) {
		...
		Integer _dId = LiquidManager.DIESEL != null ? net.minecraftforge.fluids.FluidRegistry.getRegisteredFluidIDs().get(LiquidManager.DIESEL) : null;
		if ((_dId != null && distilInventory.getLiquidItemID() == _dId)) {
			if (intersectsWith(t, g)) {
				drawCreativeTabHoveringText("Diesel", t, g);
			}
		}
		Integer _rfId = LiquidManager.REFINED_FUEL != null ? net.minecraftforge.fluids.FluidRegistry.getRegisteredFluidIDs().get(LiquidManager.REFINED_FUEL) : null;
		if ((_rfId != null && distilInventory.getLiquidItemID() == _rfId)) {
			if (intersectsWith(t, g)) {
	...
	protected void drawGuiContainerBackgroundLayer(float f, int t, int g) {
		...
		Integer _rfId2 = LiquidManager.REFINED_FUEL != null ? net.minecraftforge.fluids.FluidRegistry.getRegisteredFluidIDs().get(LiquidManager.REFINED_FUEL) : null;
		if (_rfId2 != null && distilInventory.getLiquidItemID() == _rfId2) {
