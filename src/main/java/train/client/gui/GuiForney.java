...
public class GuiForney extends GuiContainer {
	...
	protected void actionPerformed(GuiButton guibutton) {
		...
		if (guibutton.id == 3) {
			...
			//System.out.println(((EntityPlayer)loco.riddenByEntity).getDisplayName().getFormattedText().equals(loco.getTrainOwner().trim()));

			if (loco.riddenByEntity != null && loco.riddenByEntity instanceof EntityPlayer && ((EntityPlayer) loco.riddenByEntity).getDisplayName().getFormattedText().equals(loco.getTrainOwner())) {
				if ((!loco.getTrainLockedFromPacket())) {
				...
				((EntityPlayer) loco.riddenByEntity).sendMessage(new TextComponentString("You are not the owner"));
			}
		}
	}
	...
	protected void drawGuiContainerForegroundLayer(int i, int j) {
		...
		fontRenderer.drawString(loco.getName(), 37, 3, 0x000000);
		fontRenderer.drawString(loco.getName(), 39, 1, 0x000000);
		fontRenderer.drawString(loco.getName(), 37, 1, 0x000000);
		fontRenderer.drawString(loco.getName(), 39, 3, 0x000000);

		fontRenderer.drawString(loco.getName(), 38, 3, 0x000000);
		fontRenderer.drawString(loco.getName(), 38, 1, 0x000000);
		fontRenderer.drawString(loco.getName(), 37, 2, 0x000000);
		fontRenderer.drawString(loco.getName(), 39, 2, 0x000000);
		fontRenderer.drawString(loco.getName(), 38, 2, 0xd3a900);
	...
	protected void drawGuiContainerBackgroundLayer(float f, int t, int g) {
		...
		if (loco instanceof SteamTrain) {
			...
			if (LiquidManager.WATER_FILTER != null && ((SteamTrain) loco).getLiquidItemID() == net.minecraftforge.fluids.FluidRegistry.getRegisteredFluidIDs().getOrDefault(LiquidManager.WATER_FILTER, 0)) {
		...
		fontRenderer.drawStringWithShadow("Speed reduction: " + loco.getCurrentSpeedSlowDown()  + " km/h", 1, 30, 0xFFFFFF);
		...
		fontRenderer.drawStringWithShadow("Persistent UUID: " + loco.getPersistentUUID() + " - Entity UUID" + loco.getUniqueID().toString(),1,0,0xFFFFFF);
	}
}