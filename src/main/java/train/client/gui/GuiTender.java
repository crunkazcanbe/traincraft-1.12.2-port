...
public class GuiTender extends GuiContainer {
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.id == 3) {
			if(player!=null && player.getName().toLowerCase().equals(tender.getTrainOwner().toLowerCase())){
				if ((!tender.getTrainLockedFromPacket())) {
					AxisAlignedBB box = tender.getEntityBoundingBox().grow(5, 5, 5);
	...
	protected void drawGuiContainerForegroundLayer(int i, int j) {
		...
		fontRenderer.drawString(tender.getName(), 34, 1, 0x000000);
		fontRenderer.drawString(tender.getName(), 36, 3, 0x000000);
		fontRenderer.drawString(tender.getName(), 34, 3, 0x000000);
		fontRenderer.drawString(tender.getName(), 36, 1, 0x000000);

		fontRenderer.drawString(tender.getName(), 34, 2, 0x000000);
		fontRenderer.drawString(tender.getName(), 36, 2, 0x000000);
		fontRenderer.drawString(tender.getName(), 35, 3, 0x000000);
		fontRenderer.drawString(tender.getName(), 35, 1, 0x000000);
		fontRenderer.drawString(tender.getName(), 35, 2, 0xd3a900);
	...
	protected void drawGuiContainerBackgroundLayer(float f, int t, int g) {
		fontRenderer.drawStringWithShadow("UUID: " + tender.getPersistentUUID() + " - Entity UUID" + tender.getUniqueID().toString(),1,0,0xFFFFFF);
		...
		if (tender != null) {
			...
			Integer _wId = net.minecraftforge.fluids.FluidRegistry.getRegisteredFluidIDs().get(net.minecraftforge.fluids.FluidRegistry.WATER);
			if (_wId != null && tender.getLiquidItemID() == _wId) {
