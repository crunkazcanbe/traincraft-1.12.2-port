...
public class GuiBuilder extends GuiContainer {
	...
	public void initGui() {
		...
		if (!(builder).getTrainLockedFromPacket()) {
			this.buttonList.add(this.buttonLock = new GuiButton(4, var1 + 3, var2 - 30, 51, 10, I18n.format("train.unlocked.name")));
		}
		else {
			this.buttonList.add(this.buttonLock = new GuiButton(4, var1 + 3, var2 - 30, 43, 10, I18n.format("train.locked.name")));
		}
	}
	...
	protected void actionPerformed(GuiButton guibutton) {
		...
		if (guibutton.id == 4) {
			if (player != null && player.getName().toLowerCase().equals((builder).getTrainOwner().toLowerCase())) {
				if ((!(builder).getTrainLockedFromPacket())) {
					AxisAlignedBB box = (builder).getEntityBoundingBox().grow(5, 5, 5);
				...
				player.sendMessage(new TextComponentString(I18n.format("train.owner.name")));
			}
		}
	}
