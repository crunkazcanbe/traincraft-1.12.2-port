	...
	protected void drawGuiContainerBackgroundLayer(float f, int t, int g) {
		...
		if(currentKnownItem!=null && isTabRecipeOpen){
			...
			if(currentRenderTabY==120){
				...
				renderEntity = train.getEntity(mc.world);
				if(renderEntity!=null && (previousItem == null || !currentKnownItem.getRegistryName().equals(previousItem.getRegistryName()))){
				...
				if(renderEntity!=null)mc.getRenderManager().renderEntity(renderEntity, 0, 0, 0, 0, 0, false);
