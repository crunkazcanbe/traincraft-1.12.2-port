...
public class GuiJukebox extends GuiScreen {
	...
	protected void actionPerformed(GuiButton button) {
		...
		if (button.id == 3) {
			if (player != null && player instanceof EntityPlayer && player.getDisplayName().getFormattedText().equals(((AbstractTrains) jukebox).getTrainOwner())) {
				if ((!((AbstractTrains) jukebox).locked)) {
					AxisAlignedBB box = jukebox.getEntityBoundingBox().grow(5, 5, 5);
