...
public class GuiLoco2 extends GuiContainer {
	...
	protected void actionPerformed(GuiButton guibutton) throws java.io.IOException {
		...
		if (guibutton.id == 3) {
			net.minecraft.entity.Entity rider = loco.getPassengers().isEmpty() ? null : loco.getPassengers().get(0);
			if (rider instanceof EntityPlayer && ((EntityPlayer) rider).getDisplayName().getFormattedText().equals(loco.getTrainOwner())) {
				if ((!loco.getTrainLockedFromPacket())) {
				...
				((EntityPlayer) rider).sendMessage(new TextComponentString("You are not the owner"));
			}
		}
		if (guibutton.id == 4) {
			net.minecraft.entity.Entity rider4 = loco.getPassengers().isEmpty() ? null : loco.getPassengers().get(0);
			if (loco.isLocoTurnedOn()) {
				if(loco.getSpeed() <= 1){
					...
					loco.isBraking = true;
					this.initGui();
				}else if(rider4 instanceof EntityPlayer){
					((EntityPlayer)rider4).sendMessage(new TextComponentString("Stop before turning it Off!"));
				}
			}
			else {
	...
	protected void drawCreativeTabHoveringText(String str, int t, int g) {
		...
		fontRenderer.drawStringWithShadow("Owner: " + loco.getTrainOwner().trim(), startX,
				startY + 40, -1);
	}
	...
	protected void drawGuiContainerBackgroundLayer(float f, int t, int g) {
		...
		if (loco instanceof SteamTrain) {
			...
			Integer _wId2 = net.minecraftforge.fluids.FluidRegistry.getRegisteredFluidIDs().get(net.minecraftforge.fluids.FluidRegistry.WATER);
			if (_wId2 != null && ((SteamTrain) loco).getLiquidItemID() == _wId2) {
		...
		fontRenderer.drawStringWithShadow("Maximum Speed: " + (loco.getCustomSpeedGUI()) + " km/h" + " ("+(loco.getCustomSpeedGUI()+loco.getCurrentSpeedSlowDown())+")", 1, 110, 0xFFFFFF);
		fontRenderer.drawStringWithShadow("Destination: " + (loco.getDestinationGUI()), 1, 120, 0xFFFFFF);
		fontRenderer.drawStringWithShadow("UUID: " + loco.getPersistentUUID() + " - Entity UUID" + loco.getUniqueID().toString(),1,0,0xFFFFFF);
	}
}