...
public class GuiFreight extends GuiContainer {
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.id == 3) {
			if (player != null && player.getName().toLowerCase().equals(freight.getTrainOwner().toLowerCase())) {
				if ((!freight.getTrainLockedFromPacket())) {
					AxisAlignedBB box = freight.getEntityBoundingBox().grow(5, 5, 5);
					List lis3 = freight.world.getEntitiesWithinAABBExcludingEntity(freight, box);
	...
	protected void drawGuiContainerForegroundLayer(int i, int j) {
		...
		fontRenderer.drawString(freight.getName(), 10, 6, 0x404040);
	...
	protected void drawGuiContainerBackgroundLayer(float f, int t, int g) {
		fontRenderer.drawStringWithShadow("UUID: " + freight.getPersistentUUID() + " - Entity UUID" + freight.getUniqueID().toString(),1,0,0xFFFFFF);
