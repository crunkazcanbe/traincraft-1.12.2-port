...
public class RecipeBookHandler {
	...
	public RecipeBookHandler() {
		vanillaWorkTableRecipes[0] = BlockIDs.assemblyTableI.block.getTranslationKey();
		vanillaWorkTableRecipes[1] = BlockIDs.assemblyTableII.block.getTranslationKey();
		vanillaWorkTableRecipes[2] = BlockIDs.assemblyTableIII.block.getTranslationKey();
		vanillaWorkTableRecipes[3] = BlockIDs.distilIdle.block.getTranslationKey();
		vanillaWorkTableRecipes[4] = BlockIDs.openFurnaceIdle.block.getTranslationKey();
		vanillaWorkTableRecipes[5] = BlockIDs.trainWorkbench.block.getTranslationKey();
		...
		vanillaWorkTableRecipes[12] = BlockIDs.lantern.block.getTranslationKey();
		...
		vanillaWorkTableRecipes[20] = BlockIDs.switchStand.block.getTranslationKey();
	}
	...
	public static List<ITCRecipe> workbenchListCleaner(List recipeList) {
		...
		for(Object r: recipeList) {
			if (r instanceof ShapedTrainRecipes || r instanceof ShapelessTrainRecipe) {
				...
				String output = recipe.getRecipeOutput().getItem().getRegistryName().toString();
	...
	public static List<TierRecipe> assemblyListCleaner(List recipeList) {
		...
		for(Object r: recipeList) {
			if(r instanceof TierRecipe) {
				...
				String output = recipe.getOutput().getItem().getRegistryName().toString();
				if (!outputs.contains(output)) {
