		...
		if (!Minecraft.getMinecraft().ingameGUI.getChatGUI().getChatOpen()) {
			...
			if (Loader.isModLoaded("ComputerCraft") || Loader.isModLoaded("OpenComputers")) {
				if (MTCScreen.isPressed() && !FMLClientHandler.instance().isGUIOpen(GuiMTCInfo.class)) {
					if (Minecraft.getMinecraft().player.getRidingEntity() != null && Minecraft.getMinecraft().player.getRidingEntity() instanceof Locomotive) {
//&&((Locomotive)Minecraft.getMinecraft().player.getRidingEntity()).getTrainOwner().equals(Minecraft.getMinecraft().player.getDisplayName().getUnformattedText()))
						if (((Locomotive)Minecraft.getMinecraft().player.getRidingEntity()).locked) {
							if (((Locomotive)Minecraft.getMinecraft().player.getRidingEntity()).getTrainOwner().equals(Minecraft.getMinecraft().player.getDisplayName().getUnformattedText())) {
				...
				if (toggleATO.isPressed() && Minecraft.getMinecraft().player.getRidingEntity() instanceof Locomotive) {
					...
					if (train.mtcStatus != 0 && train.mtcType == 2) {
						if (!train.trainIsATOSupported()) {
							((EntityPlayer) train.getControllingPassenger()).sendMessage(new TextComponentString("Automatic Train Operation is not supported for this train"));
						} else {
						...
						((EntityPlayer) train.getControllingPassenger()).sendMessage(new TextComponentString("Automatic Train Operation can only be activated when you are using W-MTC"));
					}
				...
				if (mtcOverride.isPressed() && Minecraft.getMinecraft().player.getRidingEntity() instanceof Locomotive) {
					...
					if (train.mtcOverridePressed) {
						...
						((EntityPlayer) train.getControllingPassenger()).sendMessage(new TextComponentString("MTC has been enabled and will re-activate when the system receives new data"));
					} else {
						train.mtcOverridePressed = true;
						((EntityPlayer) train.getControllingPassenger()).sendMessage(new TextComponentString("MTC has been disabled and will not receive speed changes or transmit MTC data"));
						train.mtcStatus = 0;
						train.speedLimit = 0;
