...
public class SheetTextureStitched extends TextureStitchEvent {
	...
	public static TextureAtlasSprite[] unstitchIcons(TextureMap iconRegister, String name, int numIcons) {
		return unstitchIcons(iconRegister, name, numIcons, 1);
	}

	public static TextureAtlasSprite[] unstitchIcons(TextureMap iconRegister, String name, int columns, int rows) {
		int numIcons = rows * columns;
		TextureAtlasSprite[] icons = new TextureAtlasSprite[numIcons];
		for (int i = 0; i < numIcons; i++) {
			String texName = name + "." + i;
			icons[i]=iconRegister.registerSprite(new ResourceLocation(texName));
		}
		return icons;
	}
}
