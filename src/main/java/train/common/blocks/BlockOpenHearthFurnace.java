...
public class BlockOpenHearthFurnace extends BlockContainer {
	...
	public static void updateHearthFurnaceBlockState(boolean flag, World world, BlockPos pos, Random random) {
		...
		if (flag) {
			world.setBlockState(pos, BlockIDs.openFurnaceActive.block.getDefaultState(), 3);
		} else {
			world.setBlockState(pos, BlockIDs.openFurnaceIdle.block.getDefaultState(), 3);
		}
	...
	public void breakBlock(World world, BlockPos pos, IBlockState state) {
		if (!keepFurnaceInventory) {
			TileEntityOpenHearthFurnace furnace = (TileEntityOpenHearthFurnace) world.getTileEntity(pos);
			if (furnace != null) {
				label0: for (int l = 0; l < furnace.getSizeInventory(); l++) {
					ItemStack itemstack = furnace.getStackInSlot(l);
