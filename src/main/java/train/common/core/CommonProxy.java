...
public class CommonProxy implements IGuiHandler {
	...
	public void registerTileEntities() {
		...
		// ponytail: CE rail-accessory blocks whose Tile* classes existed but were never registered,
		// crashing on placement ("... is missing a mapping!" at TileEntity.writeToNBT). Each needs a
		// unique string id. TESRs for these are bound in ClientProxy (except MFPBWigWag, see note).
		...
		// ponytail: was unregistered -> crashed on save/sync when placed (same bug as metro pole).
	...
	public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
		...
		case (GuiIDs.TRAIN_WORKBENCH):
			return te != null && te instanceof TileTrainWbench ? new ContainerTrainWorkbench(player.inventory, player.world, (TileTrainWbench) te) : null;
		case (GuiIDs.LOCO):
		...
		case (GuiIDs.CRAFTING_CART):
			return new ContainerWorkbenchCart(player.inventory, player.world);
