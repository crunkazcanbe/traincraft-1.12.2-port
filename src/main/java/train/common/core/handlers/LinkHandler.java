...
public class LinkHandler {
	...
	public void handleStake(EntityRollingStock entityOne, AxisAlignedBB customBoundingBox) {
		...
		if (entityOne.isAttaching) {
			List lis = world.getEntitiesWithinAABBExcludingEntity(entityOne, customBoundingBox.expand(15, 5, 15));
			if (entityOne.bogieLoco != null) {
				lis.addAll(world.getEntitiesWithinAABBExcludingEntity(entityOne, entityOne.bogieLoco.getEntityBoundingBox().expand(7, 5, 7)));
			}
	...
	public void addStake(EntityRollingStock cart1, EntityRollingStock cart2, boolean byPlayer) {
		if (world.isRemote) {
			...
			if (cart2.isAttaching && cart1.isAttaching) {
				...
				if(cart1.bogieLoco!=null || cart2.bogieLoco!=null){
					...
					if(cart1.bogieLoco!=null && cart2.bogieLoco==null){
						...
						for(int i = 0; i< distancesX.length;i++){
							euclidian[i] = (float) Math.sqrt((distancesX[i] * distancesX[i]) + (distancesZ[i] * distancesZ[i]));
						}
				...
				if ((float) Math.sqrt((d * d) + (d1 * d1)) <= distanceBehindCart) {
					/**
	...
	private void StakePhysic(EntityRollingStock cart1, EntityRollingStock cart2, int linkIndex) {
		if (world.isRemote || cart1.ticksExisted<5 || cart2.ticksExisted<5) {
			return;
		}
		if (cart2.isAttached && cart1.isAttached && areLinked(cart2, cart1)) {
			...
			if(d2>20){
				...
				if(cart1.world!=null){
					EntityPlayer player = cart1.world.getClosestPlayer(cart1.posX, cart1.posY, cart1.posZ, 300, null);
					if(player!=null){
