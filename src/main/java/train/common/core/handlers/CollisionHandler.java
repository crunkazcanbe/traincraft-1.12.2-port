...
public class CollisionHandler {
	...
	public void handleCollisions(Entity entityOne, AxisAlignedBB boundingBox) {
		 ...
		 */
		listRide = world.getEntitiesWithinAABBExcludingEntity(entityOne, boundingBox.expand(-0.5, -0.5, -0.5));
		if (listRide != null && listRide.size() > 0) {
			for (int j1 = 0; j1 < listRide.size(); j1++) {
				...
				if (!(entity instanceof EntityLasersLines) && !entity.noClip) {
					if (!entity.isBeingRidden() && !(unAutorizedMob(entity, entityOne)) && (entity instanceof EntityLiving)) {
		...
		listRide = world.getEntitiesWithinAABBExcludingEntity(entityOne, box);
		if (listRide != null && listRide.size() > 0) {
			...
			for (int j1 = 0; j1 < listRide.size(); j1++) {
				...
				if (!(entity instanceof EntityLasersLines) && !entity.noClip) {
					...
					if (!entity.isBeingRidden() && (entity instanceof EntityLiving) && (!entity.getClass().getName().equals("EntityLittleMaid")) && (unAutorizedMob(entity, entityOne))) {

						applyCollisionLiving(entity, entityOne);
						// applyEntityCollision(entity, entityOne);
						return;
					}
					if (!entity.isBeingRidden() && (entity instanceof EntityPlayer) && (entityOne instanceof AbstractTrains)) {
		...
		if (listRide != null && listRide.size() > 0) {
			...
			for (int j1 = 0; j1 < listRide.size(); j1++) {
				...
				if (!(entity instanceof EntityLasersLines) && !entity.noClip && !(entity instanceof EntityLiving) && !(entityOne instanceof EntityLiving)) {
					...
					if (!entity.isBeingRidden() && entity.canBePushed() && (entityOne instanceof AbstractTrains) && (entity instanceof AbstractTrains) && !((AbstractTrains) entityOne).isAttached) {
					...
					else if (!entity.isBeingRidden() && entity.canBePushed() && (entity instanceof EntityMinecart) && !(entity instanceof AbstractTrains) && (entityOne instanceof AbstractTrains) && !((AbstractTrains) entityOne).isAttached) {
					...
					else if (!entity.isBeingRidden() && entity.canBePushed() && (entity instanceof AbstractTrains) && (entityOne instanceof AbstractTrains) && !(entity == ((EntityRollingStock)entityOne).cartLinked1) && !(entity == ((EntityRollingStock)entityOne).cartLinked2)) {
	...
	public void applyEntityCollisionVanilla(Entity par1Entity, EntityMinecart entityOne) {
		...
		if (!this.world.isRemote) {
			if (!entityOne.getPassengers().contains(par1Entity)) {
				...
				if (var6 <= ((AbstractTrains) entityOne).getLinkageDistance((EntityMinecart) entityOne)*0.8 && var6 >= 9.999999747378752E-5D) {
					...
					if (entity instanceof EntityMinecart) {
						...
						if(entity instanceof EntityPlayer){
							...
							RayTraceResult movingobjectposition = world.rayTraceBlocks(new Vec3d(entityOne.posX, entityOne.posY, entityOne.posZ), new Vec3d(entityOne.posX + entityOne.motionX, entityOne.posY + entityOne.motionY, entityOne.posZ + entityOne.motionZ), false);
							if (entity != null && entity instanceof EntityPlayer) {
								movingobjectposition = new RayTraceResult(entity);
							}
							if (movingobjectposition != null) {
								if (movingobjectposition.entityHit != null) {
									float f7 = (float) Math.sqrt(entityOne.motionX * entityOne.motionX + entityOne.motionZ * entityOne.motionZ);
	...
	public void applyRideEntity(Entity entity, Entity entityOne) {
		...
		if ((entity instanceof EntityLiving) && ((AbstractTrains) entityOne).canBeRidden() && entityOne.getPassengers().isEmpty() && entity.getRidingEntity() == null) {
	...
	public void applyCollisionLiving(Entity entity, Entity entityOne) {
		...
		if (d2 <= 0.7D && !(entityOne instanceof EntityPlayer)&& d!=0 && d1!=0) {
			...
			if (entity.getRidingEntity() == null) {
				...
				RayTraceResult movingobjectposition = world.rayTraceBlocks(vec3d4, vec3d5, false);
				...
				if (movingobjectposition != null) {
					...
					if (movingobjectposition.entityHit != null) {
						...
						float f1 = (float) Math.sqrt(entityOne.motionX * entityOne.motionX + entityOne.motionY * entityOne.motionY + entityOne.motionZ * entityOne.motionZ);
						float f7 = (float) Math.sqrt(entityOne.motionX * entityOne.motionX + entityOne.motionZ * entityOne.motionZ);
						double damage = 1D;
						f1 *= 6;//ratio
						...
						if (unAutorizedMob(movingobjectposition.entityHit, entityOne) || ((entity instanceof EntityPlayer || entityOne instanceof EntityPlayer)) && ((f1 * 3.6) > 60))// (movingobjectposition.entityHit instanceof EntityCreature) && !(movingobjectposition.entityHit instanceof EntityWolf))
						{
							...
							//world.playSoundAtEntity(entityOne, "damage.fallsmall", 1.0F, 1.2F / (rand.nextFloat() * 0.2F + 0.9F));
							entityOne.playSound(net.minecraft.util.SoundEvent.REGISTRY.getObject(new net.minecraft.util.ResourceLocation("entity.generic.hurt")), 1.0F, 1.2F / (this.rand.nextFloat() * 0.2F + 0.9F));
							for (int var9 = 0; var9 < 4; ++var9) {
								// entity.world.spawnParticle(EnumParticleTypes.CRIT, entity.posX + entity.motionX * var9 / 4.0D, entity.posY + entity.motionY * var9 / 4.0D, entity.posZ + entity.motionZ * var9 / 4.0D, -entity.motionX, -entity.motionY + 0.2D, -entity.motionZ);
							}
	...
	public void applyCollision2(Entity entity, Entity entityOne) {
		...
		if (d2 >= 0.0099999997764825821D) {
			...
			if (entity instanceof EntityMinecart) {
				...
				Vec3d vec3d1 = new Vec3d(MathHelper.cos((entityOne.rotationYaw * 3.141593F) / 180F), 0.0D, MathHelper.sin((entityOne.rotationYaw * 3.141593F) / 180F)).normalize();
