...
public class RollingStockStatsEventHandler {
	...
	public void trainDistance() {
		...
		int var7 = Math.round((float) Math.sqrt(xDist * xDist + yDist * yDist + zDist * zDist) * 100);
		...
		if (train.riddenByEntity != null && train.riddenByEntity instanceof EntityPlayer) {
			for (playerDistance d : plDist) {
				if (((EntityPlayer) train.riddenByEntity).getDisplayName().getFormattedText().equals(d.getUsername())) {
					d.setDistance(d.getDistance() + (var7));
					//System.out.println(d.getDistance()+" "+((EntityPlayer)riddenByEntity).username);
					return;
				}
			}
			plDist.add(new playerDistance(((EntityPlayer) train.riddenByEntity).getDisplayName().getFormattedText(), (var7)));
		}
	}
