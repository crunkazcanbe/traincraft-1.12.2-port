...
public class NEIAssemblyTableRecipePlugin extends ShapedRecipeHandler {
	...
	public class CachedShapedRecipe extends CachedRecipe {
		...
		public CachedShapedRecipe(ShapedRecipes recipe) {
			this(recipe.recipeWidth, recipe.recipeHeight, recipe.recipeItems.stream().map(net.minecraft.item.crafting.Ingredient::getMatchingStacks).toArray(), recipe.getRecipeOutput());
		}
		...
		public List<PositionedStack> getCycledIngredients(int cycle, List<PositionedStack> ingredients) {
			...
			for (int itemIndex = 0; itemIndex < ingredients.size(); itemIndex++) {
				...
				int[] oreIds = OreDictionary.getOreIDs(ingredients.get(itemIndex).item);
				...
				if (oreName.equals("ingotSteel") || oreName.equals("ingotIron") || oreName.equals("ingotCopper") || oreName.equals("dustPlastic") || oreName.equals("dustCoal")) {
					List list = OreDictionary.getOres(oreName);
	...
	public void drawBackground(int recipe) {
		...
		if (tierRecipe != null) {
			drawString("Tier: " + tierRecipe.getTier(), 0, -11, 0x404040);
		}
	}
