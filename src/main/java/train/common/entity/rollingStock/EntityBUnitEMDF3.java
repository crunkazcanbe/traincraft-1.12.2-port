...
public class EntityBUnitEMDF3 extends LiquidTank implements IFluidHandler {
	...
	public void onUpdate() {
		...
		if (theTank != null && theTank.getFluid() != null) {
			...
			this.dataWatcher.updateObject(4, (theTank.getFluid() != null ? 1 : 0));
	...
	protected void readEntityFromNBT(NBTTagCompound nbttagcompound) {
		...
		if (nbttagcompound.hasKey("FluidName")) {
			fill(EnumFacing.UP, FluidStack.loadFluidStackFromNBT(nbttagcompound), true);
		}
		...
		for (int i = 1; i < cargoItems.length; i++) {
			if (cargoItems[i] == null) {
				...
				if (var9 <= itemstack1.getMaxStackSize()) {
					...
					cargoItems[i].setCount(cargoItems[i].getCount() + (1));
				}
				return;
			}
	...
	public boolean processInitialInteract(EntityPlayer entityplayer, net.minecraft.util.EnumHand hand) {
		...
		if (!this.world.isRemote) {
			entityplayer.openGui(Traincraft.instance, GuiIDs.LIQUID, world, this.getEntityId(), -1, (int) this.posZ);
		}
		return true;
	}
	...
	public String getLiquidName(){
		return theTank != null && theTank.getFluid() != null ? theTank.getFluid().getFluid().getUnlocalizedName() : null;
	}
}