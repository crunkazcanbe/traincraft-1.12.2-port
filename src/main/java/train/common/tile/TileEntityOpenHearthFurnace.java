...
public class TileEntityOpenHearthFurnace extends TileTraincraft implements ITickable {
	...
	public void update() {
		...
		if (!this.world.isRemote) {
			...
			if (flag != (furnaceBurnTime > 0)) {
				flag1 = true;
				BlockOpenHearthFurnace.updateHearthFurnaceBlockState(furnaceBurnTime > 0, this.world, getPos(), random);
			}
			this.syncTileEntity();
		}
		if (this.world.isRemote) {
			if (furnaceBurnTime > 0) {
				smoke(this.world, getPos().getX(), getPos().getY(), getPos().getZ(), random);
			}
		}
		if (flag1) {
			markDirty();
	...
	private int getItemBurnTime(ItemStack it) {
		...
		if (Item.getIdFromItem(var1) < 256 && Block.getBlockFromItem(var1).getDefaultState().getMaterial() == Material.WOOD)
			return 300;
		if (var1 == Items.STICK)
			return 100;
		if (var1 == Items.COAL)
	...
	public void readFromNBT(NBTTagCompound nbt, boolean forSyncing){
		...
		int orientIndex = nbt.getByte("Orientation") & 0xFF;
		facing = orientIndex < EnumFacing.values().length ? EnumFacing.values()[orientIndex] : EnumFacing.NORTH;
