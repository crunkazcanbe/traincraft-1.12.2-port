...
public class TileInfoTransmitterSpeed  extends TileEntity implements IPeripheral, SimpleComponent {
    ...
    public void updateEntity() {
        ...
        if (isActivated) {
            List<Entity> list = this.world.getEntitiesWithinAABBExcludingEntity(null, this.getRenderBoundingBox());
			if (list.size() == 0 ) {
				hadSentPacket = false;
                    ...
                    if (obj instanceof Locomotive) {
                        ...
                        if (daTrain.mtcOverridePressed) { return;}
                         if (daTrain.mtcStatus == 0  && hadSentMTCPacket == false) {
                             daTrain.mtcStatus = 1;
                            Traincraft.mscChannel.sendToAllAround(new PacketMTC(daTrain.getEntityId(),   1, 1)  , new NetworkRegistry.TargetPoint(this.world.provider.getDimension(), daTrain.posX, daTrain.posY, daTrain.posZ, 150.0D));
                        ...
                        if (!this.world.isRemote && hadSentPacket == false) {

                            trainNumber = daTrain.getEntityId();
							hadSentPacket = true;
                            Traincraft.itsChannel.sendToAllAround(new PacketSetSpeed(setSpeed, this.getPos().getX(), this.getPos().getY(), this.getPos().getZ(), daTrain.getEntityId()) , new NetworkRegistry.TargetPoint(this.world.provider.getDimension(), daTrain.posX, daTrain.posY, daTrain.posZ, 150.0D));
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        if (boundingBox == null) {
            boundingBox = new AxisAlignedBB(getPos().getX(), getPos().getY(), getPos().getZ(), getPos().getX() + 2, getPos().getY() + 2, getPos().getZ() + 2);
        }
        return boundingBox;
    }
