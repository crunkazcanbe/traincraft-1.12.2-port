...
public class TileATOTransmitterStopPoint extends TileEntity implements IPeripheral, SimpleComponent {
   ...
   public void updateEntity() {
      if(world != null) {
         if(this.isActivated) {
            List<net.minecraft.entity.Entity> list = this.world.getEntitiesWithinAABBExcludingEntity(null, this.getRenderBoundingBox());
            ...
            for (Object obj : list) {
               if (obj instanceof Locomotive) {
                  ...
                  if (daTrain.mtcStatus == 1 | daTrain.mtcStatus == 2) {
                     ...
                     Traincraft.atoSetStopPoint.sendToAllAround(new PacketATOSetStopPoint(daTrain.getEntityId(), Double.valueOf(this.stopX), Double.valueOf(this.stopY), Double.valueOf(this.stopZ), daTrain.xStationStop, daTrain.yStationStop, daTrain.zStationStop), new NetworkRegistry.TargetPoint(this.world.provider.getDimension(), daTrain.posX, daTrain.posY, daTrain.posZ, 150.0D));
                  }
               }
            }
         }
   ...
   public AxisAlignedBB getRenderBoundingBox() {
      if (boundingBox == null) {
         ...
         boundingBox = new AxisAlignedBB(xCoord, yCoord, zCoord, xCoord + 2, yCoord + 2, zCoord + 2);
      }
      return boundingBox;
   }
