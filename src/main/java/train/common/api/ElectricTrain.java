...
public abstract class ElectricTrain extends Locomotive {
	...
	public void onUpdate() {
		...
		blocksToCheck = new TileEntity[]{world.getTileEntity(new net.minecraft.util.math.BlockPos(MathHelper.floor(posX), MathHelper.floor(posY-2), MathHelper.floor(posZ))),
		...
		for (TileEntity block : blocksToCheck) {
			if (block instanceof IEnergyHandler) {
				for (EnumFacing direction : EnumFacing.values()) {
					...
					if (max > 0) {
						draw = ((IEnergyHandler) block).receiveEnergy(direction, Math.max(-MathHelper.floor(Math.min(200, maxEnergy - getFuel()) * 0.2), -max), false);
					}
				}
				fuelTrain += -draw;
			}
