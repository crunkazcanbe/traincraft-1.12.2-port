...
public abstract class Tender extends Freight implements IFluidHandler {
	...
	public void onUpdate() {
		...
		if (theTank != null && theTank.getFluid() != null) {
			...
			Integer flId = net.minecraftforge.fluids.FluidRegistry.getRegisteredFluidIDs().get(theTank.getFluid().getFluid());
			this.dataWatcher.updateObject(4, flId != null ? flId : 0);
		}
	...
	protected void checkInvent(ItemStack tenderInvent, Tender loco) {
		...
		if(ticksExisted%5==0 && fill(EnumFacing.UP, new FluidStack(FluidRegistry.WATER,100), false)==100) {
			...
			if (drain ==null && cartLinked1 instanceof LiquidTank
				...
				if (getFluid() == null) {
					drain = ((LiquidTank) cartLinked1).drain(EnumFacing.UP, new FluidStack(FluidRegistry.WATER, 100), true);
					...
					&& !(cartLinked1 instanceof EntityBUnitEMDF7) && !(cartLinked1 instanceof EntityBUnitEMDF3) && !(cartLinked1 instanceof EntityBUnitDD35)) {
				if (getFluid() == null) {
					drain = ((LiquidTank) cartLinked2).drain(EnumFacing.UP, new FluidStack(FluidRegistry.WATER, 100), true);
				} else if (getFluid().getFluid() == FluidRegistry.WATER) {
					drain = ((LiquidTank) cartLinked2).drain(EnumFacing.UP, new FluidStack(FluidRegistry.WATER, 100), true);
				}
			}
