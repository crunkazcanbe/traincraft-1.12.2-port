...
public abstract class SteamTrain extends Locomotive {
	...
	public void onUpdate() {
		...
		if (rand.nextInt(100) == 0 && getWater() > 0 && getIsFuelled()) {
			drain(EnumFacing.UP, getWaterConsumption() / 5, true);
		}
	}
		...
		for (int i = 2; i < loco.locoInvent.length; i++) {
			if (loco.locoInvent[i] == null) {
				...
				if (var9 <= itemstack1.getMaxStackSize()) {
					loco.locoInvent[i].setCount(var9);
					return;
				}
				else if (locoInvent[i].getCount() < locoInvent[i].getMaxStackSize()) {
					loco.locoInvent[i].grow(1);
					return;
				}
			}
	...
	protected void checkInvent(ItemStack locoInvent0, ItemStack locoInvent1, SteamTrain loco) {
		...
		if (isLocoTurnedOn() && ticksExisted%10==0) {
			...
			if(fill(EnumFacing.UP,new FluidStack(FluidRegistry.WATER, 100), false)==100) {
				...
				for (TileEntity block : blocksToCheck) {
					if (drain == null && block instanceof IFluidHandler) {
						FluidStack testDrain = ((IFluidHandler) block).drain(100, false);
						if (testDrain != null &&
								testDrain.getFluid() == FluidRegistry.WATER &&
			...
			if(cartLinked1 instanceof Tender){
				if(drain==null && fill(EnumFacing.UP,new FluidStack(FluidRegistry.WATER, 100), false)==100) {
					if (getFluid() == null || getFluid().getFluid() == FluidRegistry.WATER) {
						drain = ((Tender) cartLinked1).drain(EnumFacing.UP, new FluidStack(FluidRegistry.WATER, 100), true);
					}
				}
				...
				if(drain==null && fill(EnumFacing.UP,new FluidStack(FluidRegistry.WATER, 100), false)==100) {
					if (getFluid() == null || getFluid().getFluid() == FluidRegistry.WATER) {
						drain = ((Tender) cartLinked2).drain(EnumFacing.UP, new FluidStack(FluidRegistry.WATER, 100), true);
					}
				}
