...
public class ServerLogger {
        ...
        try {
            ...
            if (wagon instanceof IInventory) {
                ...
                if (wagon instanceof LiquidTank) {
                    for (FluidTankInfo tank : ((LiquidTank)wagon).getTankInfo(EnumFacing.UP)) {
    ...
    private static void addFluidXML(StringBuilder string, FluidStack item){
        // FluidContainerRegistry removed in 1.12.2 - fluid logging skipped
    }
    ...
    public static List<ItemStack> getItems(String doc){
        try {
            ...
            itemStacks.add(new ItemStack(ForgeRegistries.ITEMS.getValue(new ResourceLocation(doc.substring(doc.indexOf("<delegate>")+10, doc.indexOf("</delegate>"))))));
    ...
    public static ItemStack parseItemFromXML(String doc){
        try {
            ItemStack stack = new ItemStack(
                    ForgeRegistries.ITEMS.getValue(new ResourceLocation(doc.substring(doc.indexOf("<delegate>")+10, doc.indexOf("</delegate>")))),//get item by delegate name since it's static
